...
auto check_soa_derives_particle_layout() -> CaseResult {
	return CaseResult{
	    ...
	    .passed =
	        starter::axis_count == 3 && sizeof(starter::ParticleSoa) == sizeof(float) * 2 * starter::axis_count * starter::soa_capacity,
	};
}
...
auto check_view_follows_declaration_order() -> CaseResult {
	...
	return CaseResult{
	    ...
	    .passed = soa.position.x[3] == 1.0f && soa.position.z[7] == 2.0f && soa.velocity.x[11] == 3.0f && soa.velocity.z[13] == 4.0f &&
...
auto agrees_with_scalar_reference(Kernel kernel, std::size_t count) -> bool {
	...
	for (auto axis = std::size_t{0}; axis < starter::axis_count; ++axis) {
		for (auto i = std::size_t{0}; i < count; ++i) {
			auto const position_difference = subject_view.position[axis][i] - reference_view.position[axis][i];
			auto const velocity_difference = subject_view.velocity[axis][i] - reference_view.velocity[axis][i];
			if (!(std::abs(position_difference) <= tolerance) || velocity_difference != 0.0f) {
				return false;
			}
		}
	}
...
auto kernel_matches(Kernel kernel) -> bool {
	return agrees_with_scalar_reference(kernel, test_count) && agrees_with_scalar_reference(kernel, starter::soa_capacity);
}

auto check_kernels_agree() -> CaseResult {
	auto passed = kernel_matches([](starter::KinematicView const& view, float step) {
		starter::integrate_loop(view, step);
	});
	passed = passed && kernel_matches([](starter::KinematicView const& view, float step) {
