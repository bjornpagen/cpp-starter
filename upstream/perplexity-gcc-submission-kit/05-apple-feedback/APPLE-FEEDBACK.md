...
File this report after the GCC patch appears in the public mailing-list archive.

Replace `DARWIN_PR_NUMBER` and `PATCH_ARCHIVE_URL` with the recorded public
values before filing. Stop if either placeholder is still present.
...
Attach `rsize.cc` if Feedback Assistant requests a source file.
