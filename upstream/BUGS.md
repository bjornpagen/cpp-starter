...
|---|---|---|---|---|---|
| [126782](https://gcc.gnu.org/bugzilla/show_bug.cgi?id=126782) | `[Darwin] sys/_types/_rsize_t.h does not define rsize_t with -fmodules` | target | UNCONFIRMED | `awaiting-regression-baseline` | The tested source diff and draft mail are in [`gcc-fixincludes-darwin-rsize-t`](gcc-fixincludes-darwin-rsize-t/). Run and compare a pre-patch full testsuite on the same host and configuration, or document a suitable `gcc-testresults` comparison. Then update the public test statement, regenerate the mail patch, and request approval. Do not comment on Bugzilla before the gcc-patches archive shows the mail. |
